                ...
                <tbody>
                    @foreach ($scheduleData as $data)
                        <tr>
                            <td>{{ $sl++ }}</td>
                            <td><a
                                    href="{{ route('admin.spacific.schedule.appointment', $data->id) }}">{{ $data->set_date }}</a>
                            </td>
                            ...
                            <td>
                                @if ($data->status == 1)
                                    <a href="{{ route('admin.schedule.status', $data->id) }}"
                                        class="badge badge-success badge-shadow" style="text-decoration: none;">Active</a>
                                @else
                                    <a href="{{ route('admin.schedule.status', $data->id) }}"
                                        class="badge badge-danger badge-shadow" style="text-decoration: none;">Inactive</a>
                                @endif
                            </td>
                            <td>
                                <a href="{{ route('admin.schedule.delete', $data->id) }}"
